...
class TestMatmulHX(TestMatmulPyTorch):
    ...
    """
    matmul: ClassVar = partial(hxtorch.perceptron.matmul, wait_between_events=10)
...
class TestMatmulHXmock(TestMatmulPyTorch):
    ...
    Tests the hxtorch matmul operation.
    """
    matmul: ClassVar = partial(hxtorch.perceptron.matmul, mock=True)
    noise_std: ClassVar[float] = hxtorch.perceptron.constants.defaults.noise_std
    gain: ClassVar[float] = hxtorch.perceptron.constants.defaults.gain

    @classmethod
    def setUpClass(cls):
        hxtorch.perceptron.set_mock_parameter(hxtorch.perceptron.MockParameter(
